...
class Game:
    ...
    def __init_images(self):
        ...
        self.__images = {
            CheckerType.RED_REGULAR: ImageTk.PhotoImage(
                Image.open(Path('assets', 'RR.png')).resize((CELL_SIZE, CELL_SIZE), Image.ANTIALIAS)),
            CheckerType.BLUE_REGULAR: ImageTk.PhotoImage(
                Image.open(Path('assets', 'BR.png')).resize((CELL_SIZE, CELL_SIZE), Image.ANTIALIAS)),
            CheckerType.RED_QUEEN: ImageTk.PhotoImage(
                Image.open(Path('assets', 'RQ.png')).resize((CELL_SIZE, CELL_SIZE), Image.ANTIALIAS)),
            CheckerType.BLUE_QUEEN: ImageTk.PhotoImage(
                Image.open(Path('assets', 'BQ.png')).resize((CELL_SIZE, CELL_SIZE), Image.ANTIALIAS)),
        }
    ...
    def mouse_down(self, event: Event):
        ...
        if (self.__field.type_at(x, y) in player_checkers) or (self.__field.type_at(x, y) in BLUE_CHECKERS):
            ...
            if (move in self.__get_moves_list(PLAYER_SIDE)) or (move in self.__get_moves_list(SideType.BLUE)):
    ...
    def __handle_enemy_turn(self):
        ...
        if random_int < ENEMY_OUR_CHECKER_MOVE_CHANCE:
            optimal_moves_list = self.__predict_bad_moves_enemy(SideType.opposite(SideType.BLUE))

        else:
            optimal_moves_list = self.__predict_optimal_moves(SideType.opposite(PLAYER_SIDE))

        print(optimal_moves_list)
    ...
    def __predict_optimal_moves(self, side: SideType) -> list[Move]:
        ...
        if (optimal_moves):
            ...
            for move in choice(optimal_moves):
                if (side == SideType.RED and self.__field.type_at(move.from_x, move.from_y) in BLUE_CHECKERS):
                    break
                elif (side == SideType.BLUE and self.__field.type_at(move.from_x, move.from_y) in RED_CHECKERS):
                    break
    ...
    def __predict_bad_moves_enemy(self, side: SideType) -> list[Move]:
        '''Предсказать худший ход для соперника'''
        best_result = 1000
        optimal_moves = []
        predicted_moves_list = self.__get_predicted_moves_list(side)
